    ...
    for x in songNotes:
        ...
        for y in x:
            ...
            if not y[0]=='z':
                ...
                if y[0]=='_':
                    result.append(base(y[0]+y[1]))
                else:
                    result.append(base(y[0]))
                # A note is found, break the loop
                break
        # Edge case: no note is found, the bar is completely silent, so just play a C.
...
def progression(note,length):
    ...
    for x in range(length*2):
        if len(chord)==2:
            ...
            # Length 8 means the whole arp takes 1/8 note, write /8 for eighth time
            for y in chord:
