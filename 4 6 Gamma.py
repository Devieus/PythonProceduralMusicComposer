...
def leadBar(state,bassBar):
    ...
    while x>0:
        fitting=False
        while not fitting:
            noteLength=int(abs(r.gauss(0,1.5))) if note!="z" else int(abs(r.gauss(0,1.5))+1)
            if noteLength<=x:
                fitting=True
                x-=noteLength
        if noteLength==0:
...
def drumriff():
    ...
    remnant=int((M/M2)/(2*L/8))-1 # The second 2 is there because of the accents. Ideally there'd be more accents. The first 2 is to have 4 beats in 4/4.
    while remnant>0:
        remnant-=1
        reps=r.choice([1,2]) # There are various things a gap could be filled in with.
