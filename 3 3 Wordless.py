...
statesDic={'0':{'name':'M','notes':[0,4,7],'next':['1']}, # State 1 will always be the tonic.
           ...
           '2':{'name':'M','notes':[0,4,7],'next':['4','5','6','7','8']}, # State 3 will feature minors.
           '3':{'name':'m','notes':[0,3,7],'next':['4','5','6','7','8']},# It might also have others later on.        
           '4':{'name':'M','notes':[0,4,7],'next':['4','5','6','7','8','0']}, # State 4 will feature majors chords.
           '5':{'name':'m','notes':[0,3,7],'next':['4','5','6','7','8','0']},# Minors chords.
           '6':{'name':'Aug','notes':[0,4,8],'next':['4','5','6','7','8','0']}, # Augmented chords.
           '7':{'name':'Dim','notes':[0,3,6],'next':['4','5','6','7','8','0']},# And diminished chords, and can jump out if it has to.
           '8':{'name':'M','notes':[0,4,7],'next':['0']}} # State 5 will bring it back to the tonic.
...
def arpeggio(chord,method,reps):
    ...
    # One edge case.
    if M==2 and reps==2:
        result+=notes[chord[0]]+"["+notes[chord[1]]+notes[chord[2]]+"]"
        result+=notes[chord[0]]+"["+notes[chord[1]]+notes[chord[2]]+"]"
        return result
    ...
    for x in range(reps):
        ...
        if method==0:
            ...
            for x in range(M):
                ...
                result+=notes[chord[x]]+length if x<3 else notes[chord[M-(x+M%4)]]+length
        if method==1:
            # Go down, then up.
            for x in range(M):
                # This method is much the same, but in reverse order.
                result+=notes[chord[M-(x+M-2)]]+length if x<3 else notes[chord[x-2]]+length
        if method==2:
            # Go up, stay up.
            # This one places the first few notes, then let the last linger.
            result+=notes[chord[0]]+length+notes[chord[1]]+length+notes[chord[2]]+str((len(length)+1)*M-2) if M>2 else notes[chord[0]]+length+notes[chord[1]]+length
        if method==3:
            # Go down, stay down.
            ...
            # This method is much the same as the third.
            result+=notes[chord[0]]+length+notes[chord[1]]+length+notes[chord[2]]+str((len(length)+1)*M-2) if M>2 else notes[chord[0]]+length+notes[chord[1]]+length
        if method==4:
            # Play the first note, then the other two; repeat.
            for x in range(M):
                # This is similar to the edge case.
                result+=notes[chord[0]]+length if x%2==1 else "["+notes[chord[1]]+length+notes[chord[2]]+length+"]"
        if method==5:
            # Play the first note, then the other two over and over.
            for x in range(M):
                result+=notes[chord[0]]+length if x==0 else "["+notes[chord[1]]+length+notes[chord[2]]+length+"]"
        if method==6:
            # Play the first note, then the other two, holding them.
            result+=notes[chord[0]]+length+"["+notes[chord[1]]+str((len(length)+1)*M-1)+notes[chord[2]]+str((len(length)+1)*M-1)+"]"
    return result
...
chorusDic["lead"]=leadify(state,chorusDic["bass"])
...
otherDic["lead"]=leadify(state,otherDic["bass"])
...
for x in range(otherLength):
    otherDic['voice'].append([])
    for y in otherDic['voiceCount'][x]:
        otherDic['voice'][x].append(word(y))
...
#    songDic["drum2"].append(otherDic["drum2"])
    songDic["drum1"].append("z"+str(M*2)) if x<otherLength*2/3 else songDic["drum1"].append(otherDic["drum1"])
    songDic["drum2"].append("z"+str(M*2)) if x<otherLength*2/3 else songDic["drum2"].append(otherDic["drum2"])
    ...
    songDic["arp"].append(["z"+str(M*2)]) if x<otherLength/3 else songDic["arp"].append(otherDic["arp"][x])
...
for x in range(songLength):
    ...
    for x in range(chorusLength):
        ...
        songDic["chord"].append(chorusDic["chord"][x])
        songDic["chordNotes"].append(chorusDic["chordNotes"][x])
        songDic["arp"].append(chorusDic["arp"][x])
        songDic["voiceCount"].append(chorusDic["voiceCount"][x])
        songDic["voice"].append(chorusDic["voice"][x])
...
#    songDic["drum1"].append(otherDic["drum1"])
#    songDic["drum2"].append(otherDic["drum2"])
    songDic["drum1"].append("z"+str(M*2))
    songDic["drum2"].append("z"+str(M*2))
    songDic["state"].append(otherDic["state"][x])
    songDic["lead2"].append(["z"+str(M*2)])
    songDic["chord"].append(otherDic["chord"][x])
    songDic["chordNotes"].append(otherDic["chordNotes"][x])
...
for i in songDic["bars"]:
    ...
    output+='z'+str(M*2-1)+drumDic['hatopen']+'|' if songDic['state'][i]=='0' and i>1 else 'z'+str(M*2)+'|'
output+=']\n\n'
