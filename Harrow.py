...
scales=['Niaventi','Ousak','Sabach','Souzinak']
# Dev is my personal scale that drops the septet altogether, because I dislike the B sound in C
scalesDic={
    'Niaventi':[0,3,4,6,7,10,11,12,15,16,18,19,22,23,-1],
    'Ousak':[0,2,4,5,7,9,11,12,14,16,17,19,21,23,-1],
    'Sabach':[0,2,4,6,7,8,11,12,14,16,18,19,20,23,-1],
    'Souzinak':[1,2,4,6,7,10,11,13,14,16,18,19,22,23,-1]
}
...
def bar():
    ...
    while(x>0):
        ...
        if r.randint(1,10)<4:
            # The rest is the last item of notes
            note=notes[-1]
        else:
            # If it's a note, the next note is one at most 5 away from the current note
            y=abs(scalesDic[scale].index(notes.index(note))+r.randint(-5,+5))
            # It has to still be on the scale list, and should also skip the rest.
            if y>len(scalesDic[scale])-2:
                # len() of a list gives the last index+1,
                # but I don't want the rest, so len-2 is the last actual note on the scale.
                y=len(scalesDic[scale])-2
            note=notes[scalesDic[scale][y]]
            # A backup of the note is stored, because it's not a rest.
            note2=note
        # Now pick a note that's up to 5 notes away in the scale, e.g. C to G
...
output+='z'+str(M*2)+'|z'+str(M*2)+'|z'+str(M*2)+'|z'+str(M*2)+'|'+progNotes[4]+'|\n'
for i in range(lines-1):
    for j in range(5):
        for k in progNotes[j+(i+1)*5]:
        # Add each bar of the note list to the output
