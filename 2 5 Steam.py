...
def bassify(state):
    ...
    for i in range(bars):
        ...
        songDic["drum1"].append(True) if state=='0' else songDic["drum1"].append(False)
        # Change the state.
...
# Fourth voice (also drums, but different).
output+='V:4\n'
output+='z'+str(M*2)
roll=drumBase()
for i in range(1,bars-1):
...
output+='V:5\n'
# This voice's only task is to put a crash (or an open hi-hat as analog) at the start of each progression cycle.
for i in range(bars):
    # It'll do this throughout the song, disregarding intros and outros.
    output+=drumDic['hatopen']+str(M*2)+'|' if songDic['drum1'][i] else 'z'+str(M*2)+'|'
output+=']'
