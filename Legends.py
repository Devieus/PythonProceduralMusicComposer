...
# For using a chorus, I remove some lines.
chorusSize=int(r.gauss(4,0.25)+0.5)
newLines=lines-(lines%chorusSize)*4
# Default note length
L=1 # eighths
L2=8
# Voice
...
for i in range(newLines):
    ...
    if i%chorusSize==chorusSize-1:
        # Add a chorus, which might only be done by V1 for now.
        for j in range(chorusSize):
            for k in range(5):
                for l in chorus[k+j*5]:
                    output+=l
                    # This line might be important.
                    songNotes2.append('z'+str(M*2))
                output+='|'
            output+='\n'
    else:
        for j in range(5):
            # Randomly add a bar to either V1 or V2.
            if r.choice([True,False]):
                for k in songNotes[j+i*5]:
                    # Add each bar of the note list to the output.
                    output+=k
                # That exact same bar has to be silent on the other voice.
                songNotes2.append('z'+str(M*2))
            else:
                output+=('z'+str(M*2))
                bar=[]
...
for i in range(newLines):
    ...
    if i%chorusSize==chorusSize-1:
        ...
        for j in range(5):
            # All the notes that didn't get put into V1 are now here.
            for k in songNotes2[j+i*5]:
            # Add each bar of the note list to the output.
                output+=k
            # And put a | delimiter at the end of it.
            output+='|'
