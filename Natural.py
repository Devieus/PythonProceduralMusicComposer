...
drum=['hatclose','clave','tomlow','tommidlow','tommid','tommidhi','tomhi']
...
def drumriff():
    ...
    # Generate a drum riff
    # A riff is comprised of a few elements, usually between 2 and 4 and are based on one of the 40 rudiments
    # These have funny names like paradiddle, flam and pataflafla
    # while doable, programming 40 rudiments is not a good starting point for procgen.
    # Still, they're good building blocks when torn apart, like a drag is 2 grace notes, a flam is 1.
    # A diddle is 2 sixteenths and a paradiddle is 4. So that's a good way to reduce the set pieces from 40 to about 6.
    # (there's also the roll and the nameless triplets (3 )
    # This is all done on the snare drum, so the bass drums, hats and clave will have to synergize for accents.

    # The way these are generated is by bar, some starting with an accent hit (like the diddles),
    # others with a grace note (like the cues).
    # The output of this would then be |h i j j| for the paradiddle and |{h h} (3 i j k l| for the single ratamacue.
    # These will then be filled out accordingly where the next drum cannot be the previous drum, so it's good there's 2 snare drums.
    # Most likely the accent drum is the bass drum for the first few times.
    return result
...
output+='V:3\n'
# Make a fresh start.
songDic["lead3"]=leadify(songDic['state'],songDic["bass"])*2
for i in songDic["bars"]:
