...
def leadBar(state,bassBar):
    ...
    while x>0:
        ...
        if r.randint(1,10)<4:
            ...
            y=abs(deltaScale.index(notes.index(note))+int(r.gauss(0,0))) # abs() bounces negatives up.
...
# Second voice, time for a bit of chaos.
output+='V:2\n'
# Don't use the first and last three bars for an intro/outro.
output+='|z'+str(M*2)+'|z'+str(M*2)+'|z'+str(M*2)+'|'
for i in range(3,bars-3):
...
song.parts[0].transpose(transpose[K]-12,True)
