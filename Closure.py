...
# Dynamic scale generation for mid-change.
...
statesDic={'0':{'name':'M','notes':[0,0,0],'next':['1']}, # State 1 will always be the tonic.
           ...
           '2':{'name':'M','notes':[0,0,0],'next':['4','2','3','8']}, # State 3 will feature majors,
           '3':{'name':'m','notes':[0,-1,0],'next':['4','2','8']}, # as well as minors and can jump out.
...
def bassBar(chord):
    ...
    while x>0:
        ...
        # It has to fit.
        while not fitting:
            # A random note length that goes up to a little over a half bar.
            noteLength=int(abs(r.gauss(0,((M/M2)/(L/L2))/2))+1)
...
def arpeggio(chord,method):
    ...
    # One edge case.
    if M==2 and reps==2:
        result+=notes[chord[0]+basic[0]]+str(L2/8)+"["+notes[chord[1]+basic[1]]+str(L2/8)+notes[chord[2]+basic[2]]+str(L2/8)+"]"
        result += notes[chord[0]+basic[0]]+str(L2/8)+"["+notes[chord[1]+basic[1]]+str(L2/8)+notes[chord[2]+basic[2]]+str(L2/8)+"]"
        return result
    ...
    for y in range(reps):
        ...
        if method==4:
            ...
            result+="["+notes[chord[0]+basic[0]]+str((M/M2)/(L/L2)/(2*reps))+notes[chord[1]+basic[1]]+str((M/M2)/(L/L2)/(2*reps))+notes[chord[2]+basic[2]]+str((M/M2)/(L/L2)/(2*reps))+"]"+"z"+str((M/M2)/(L/L2)/(2*reps))
        if method==5:
...
for x in range(chorusLength):
    for y in chorusDic['voiceCount'][x]:
        chorusDic['voice'].append(abcutil.word(y))
