...
# Determine the length of the chorus in lines.
chorusSize=max(int(r.gauss(2,0.25)+0.5),2)
# Determine the number of lines between choruses.
chorusInterval=max(int(r.gauss(4,0.25)+0.5),3)
# If lines=10 and chorusSize=2 and chorusInterval=4, lines 4,5,8,9 are chorus
for x in range(lines):
    if x%chorusInterval==chorusInterval-1:
        lines-=chorusSize
...
def bar():
    ...
    # If the next note is a rest, use the previous note that was a note to determine what the next note can be.
    backupNote=note
    while(x>0):
        note2=''
        braces=['','']
        ...
        if r.randint(1,10)<4:
            ...
            # If it's a note, check to see if any other notes'll be attached to it.
            note2='' # If there ain't, it won't affect anything.
            if r.choice([True,False]):
                # Make it just 2 for now
                note2=notes[r.choice(scalesDic[scale])]
                braces=['[',']']
            # The next note is one at most 5 away from the current note, bounce up anything below 0. 
        ...
        if(notelength>1):
            ...
            result.append(braces[0]+note2+note+str(notelength)+braces[1])
        else:
            result.append(braces[0]+note2+note+braces[1])
...
# A routine that makes x lines of music
def songify(x):
    result=[]
    # x bars per song
    for k in range(x*5):
...
def bass(note):
    ...
    if length!=M*2:
        ...
        # Make a new note that's one octave lower
        altNote=note+','
        # Fill up the bar
        flag=True
...
'''
# Generate a song.
chorus=songify(chorusSize)
songNotes=songify(lines)
songNotes2=[]
# The first lead.
output+='V:1\n'
# This is for outputting the song notes.
for i in range(lines):
    # Check to see if a chorus needs to be added.
    if i%chorusInterval==chorusInterval-1:
        # For the duration of the chorus,
        for j in range(chorusSize):
            # For all 5 bars of a line.
            for k in range(5):
                # I'm delegating the chorus to V2.
                output+='z'+str(M*2)+'|'
            output+='\n'
    else:
        for j in range(5):
            # Add each bar of the note list to the output, and a bar delimiter.
        ...
        if i*j<(lines-1)*4:
            output+='\n'
# voice end
output+='z'+str(M*2)+'|z'+str(M*2)+'|]\n\n'

# Second lead, using what the first lead doesn't.
output+='V:2\n'
# This is for outputting the song notes.
for i in range(lines):
    # Check to see if a chorus needs to be added.
    if i%chorusInterval==chorusInterval-1:
        # Add a chorus, which V2 does exclusively.
        for j in range(chorusSize):
            for k in range(5):
                for l in chorus[j*5+k]:
                    output+=l
                output+='|'
            output+='\n'
    else:
...
for i in range(lines-1):
    # Check to see if a chorus needs to be added.
    if i%chorusInterval==chorusInterval-2:
        # Add a chorus.
        for j in range(chorusSize):
            for k in range(5):
                for l in bassChorus[k+j*5]:
                    output+=l
                output+='|'
            output+='\n'
    # No chorus has to be added just yet, resume normal operations.
    else:
        for j in range(5):
...
for i in range(lines-1):
    # Check to see if a chorus needs to be added.
    if i%chorusInterval==chorusInterval-2:
        # Add a chorus, which might only be done by V1 for now.
        output+=drumChorus
    else:
